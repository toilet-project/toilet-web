...
**이 단계 직후 남아 있던 작업**: 새 API projection/sitemap 통합, 상시 캐시 갱신 key/outbox 연결, 모바일 전체 회귀·신규 동의/제보 복귀·부하 검증, 최종 운영 전환. 목록/상세의 구분 표기 차이도 후속 검토 대상으로 기록했다. API 통합과 구분 표기 수정은 아래 최신 결과에서 완료했다.

무료 Workers 유지. preview는 운영 API를 공유하므로 이후 제보 등의 쓰기는 실제 운영 데이터에 영향을 준다. 사용자 인증 정보는 읽거나 기록하지 않았다.

## 최신 후속 — 지역정보·사이트맵 통합 완료

2026-09-05 사용자 승인으로 API PR #81을 운영 배포하고 preview를 `f4ed27b`로 갱신했다. 53,582개 API ID와 6개 sitemap XML의 URL을 대조했고, 지역정보 5개 표본의 실제 서버 HTML/metadata/JSON-LD가 일치했다. 단일 목록의 구분값 누락도 수정·브라우저 확인했다.

상세 근거·배포 식별자·제한사항·남은 작업은 [통합 검증 보고서](nextjs-seo-integration-verification.md)에 정리한다. 상시 outbox 연동·전체 모바일 회귀·부하 및 본 도메인 전환은 아직 완료하지 않았다.
