...
2. SEO API #194와 웹 #287을 먼저 적용한다. API #195의 V6 수동 SQL을 설치·검증하고 그룹·좌표 범위가 outbox에 남는지 확인한다. API #195 코드를 기존 v2 계약으로 적용한 뒤, 웹 #289의 v3 수신기와 30일 TTL을 적용한다. 이때 v2 알림은 전체 지역을 갱신한다. 웹 수신·지역 이동 표본을 확인한 뒤 API 계약 값을 3으로 전환한다. 좌표 이동은 이전·새 지역을 모두 재조회하게 해야 한다.
