...
| 사이트맵 | 공개 ID를 `toilet-catalog` 태그로 1시간 캐시, XML은 공유 캐시 5분. | 번역·개방시간 변경 트리거가 현재 `catalog_changed=true`를 넣어 공개 ID가 그대로여도 사이트맵을 재생성한다. |
| 리뷰·사진·계정 | 리뷰 목록은 브라우저 30초, 목록 평점은 1분. 공개 프로필 사진은 별도 CDN 정책. 로그인·관리자 읽기는 캐시하지 않는다. | 시설 캐시와 섞지 않는다. 리뷰·사진 변경은 각자의 무효화 경로를 유지한다. |

API v2 이벤트는 시설 ID·revision·action·`catalogChanged`만 전달한다. DB의 `toilet_translation`과 `toilet_opening_hours` 트리거는 이벤트를 보내지만, display-group 번역·멤버 변경은 이 경로에 없고, 변경 **전** 지역 코드도 전달되지 않는다. outbox 전달 지연·실패가 생기면 30일 캐시가 오래된 화면을 지속할 수 있으므로, 새 캐시 확대 전에 전달 상태를 먼저 검증해야 한다. 공유 캐시 플래그를 끄는 것만으로 현재 30일 Next fetch/페이지 TTL이 1시간으로 바뀌지는 않는다.
...
| 구·군 목록 `region-markers:{sigunguCode}` | 해당 다각형 안의 시설 ID·좌표·그룹·모든 공개 번역이 들어간 서버 지도 응답 | 지역 변경 이벤트를 모두 연결한 뒤 Next 데이터 캐시를 **30일**로 확대하고 `region:{code}:markers` 태그 추가. 언어별로 원본 응답을 중복 저장하지 않음. 변경 시 이전·새 구·군을 모두 만료. 브라우저로 보낼 때는 선택 언어와 한국어 대체 표시에 필요한 필드만 투영. |
| 지역 페이지 `page:{build}:{locale}:region:{code}` | 지도와 언어별 제목·문구·메타데이터 | 마커·시설 수 무효화가 연결된 뒤 ISR을 **30일**로 확대. 영향받은 구·군의 6개 언어 경로만 만료. 전국/시·도는 집계가 변할 때만 만료. |
...
| 사이트맵 `toilet-catalog` | 공개 시설 ID 및 shard | 공개 ID 집합이 달라질 때만 만료. 1시간 원본 + XML 5분 유지. |
...
언어 코드는 `ko`, `en`, `ja`, `zh-cn`, `zh-tw`, `zh-hk`로 정규화한다. 공유 시설·지역 마커 **데이터**는 언어 중립이며, 페이지와 메타데이터는 언어별이다. 특정 번역이 빠졌을 때의 한국어 대체 표시와 중국어 지역 간 임의 혼용 금지는 현 규칙을 유지한다. 번역 변경 이벤트는 안전하게 모든 언어의 영향을 받는 페이지를 만료할 수 있지만, sitemap과 전국 지도 전체를 매번 지우지 않는다.
...
지역 정보가 드물게 바뀐다는 점에는 동의한다. 현행 1시간은 지역 지도 기능을 추가할 때 지정한 값이며, 별도의 근거 문서는 확인하지 못했다. 현행에는 지역 무효화가 없어서 1시간이 변경 반영의 안전망이지만, 지역·그룹·번역 이벤트와 운영 감시가 완성되면 마커 데이터와 언어별 지역 페이지를 30일로 늘리는 편이 적절하다. 그 전에는 TTL만 먼저 늘리지 않는다. 상세 시설과 마찬가지로 변경 시 즉시 태그·경로를 만료하고, 알림 장애 때 짧은 TTL로 되돌릴 수 있어야 한다.
...
flowchart LR
    ...
    O -->|revision·범위·이전/새 지역| W[서명된 Web 수신기]
    W --> D
    W --> G
    W --> T[D1 태그/경로 만료]
    T --> P
...
| --- | --- | --- | --- |
| 시설 이름·주소·유형·설비 | 해당 ID + 6개 언어 일반/지역 상세 경로. 한국어 이름 변경 시 이전/새 slug 처리 | 지도 표시 필드 변경이면 현재 구·군 | 공개 ID가 그대로면 사이트맵 유지 |
| 좌표 이동·지역 판정 변경 | 해당 ID, 이전/새 지역 시설 경로와 메타데이터 | **이전과 새 구·군** 목록. 경계 출입으로 공개 시설 수가 달라지면 해당 구·군·시·도·전국 집계 | 공개 ID가 그대로면 사이트맵 유지 |
| 시설 번역 추가·수정·삭제 | 해당 ID의 공유 객체와 해당 언어 페이지. 누락 번역의 대체 표시가 달라질 수 있으면 전체 언어 페이지 만료 | 현재 구·군 마커 이름 | 사이트맵 유지 |
| 그룹 이름·번역·멤버·좌표 변경 | 현재 상세에 그룹명이 없으므로 상세 무효화 불필요 | 그룹의 이전/새 멤버가 속한 구·군 목록. 전국 일반 지도는 다음 직접 조회에서 반영 | 사이트맵 유지. 그룹·멤버 변경을 잡는 이벤트 추가 |
| 개방시간·24시간 여부 | 해당 ID 상세와 언어별 페이지 | 현재 구·군 지도 응답에 개방시간 필드는 없으므로 현 구조에서는 불필요. 향후 `open24h` 지역 필터가 생기면 그 변형만 만료 | 공개 ID가 그대로면 사이트맵 유지 |
| 공개 시설 추가·삭제·비공개/복구 | 해당 ID의 음수 캐시/tombstone과 상세 경로 | 이전/새 구·군 목록·시설 수 | 공개 ID가 바뀌므로 사이트맵 만료 |
...
현재 지역 시설 URL은 **한국어 원본 이름으로 만든 slug**와 좌표의 지역 코드에 의존한다. 따라서 번역만 바뀔 때 slug는 그대로이고 페이지 내용만 만료하면 된다. 이름·좌표가 바뀌는 경우에는 이전 URL도 만료해 새 URL로의 리다이렉트/404가 오래된 HTML에 가려지지 않도록 한다.

## 이벤트와 일관성 계약

v2 수신기를 유지한 채 v3에서 `entity`, `scopes`, `affectedDistrictCodes`, `revision`을 더하는 점진적 계약을 제안한다. `catalogChanged`는 신규·삭제·공개 여부처럼 **공개 ID 집합** 변경에만 쓴다. 예를 들어 시설 번역은 `DETAIL`, `REGION_MARKERS` 범위이고 사이트맵 범위가 아니다. 그룹 변경은 그룹 자체 이벤트이거나 멤버 ID별 이벤트로 기록하되, 그룹 삭제 전에 영향 멤버·지역을 먼저 기록한다.
...
2. 지역 코드별 데이터 태그와 6개 언어 지역 페이지 경로를 수신기에 연결한다. 좌표 이동의 이전/새 지역, 그룹 이름·멤버 변경, 시설 번역 변경을 격리 DB·Web 검사로 검증한다.
...
5. 프리뷰에서 한 구·군의 한국어/영어/일본어/중국어 세 지역 페이지를 순회해 이름·그룹·주소 대체 표시를 확인한다. 시설의 구·군 이동은 이전 지도에서 제거되고 새 지도에 한 번만 나타나야 한다. 24시간·삭제·번역 수정·그룹 수정·slug 변경·사이트맵 불필요 갱신 없음도 확인한다.
