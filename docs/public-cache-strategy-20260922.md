...
| 구·군 목록 `region-markers:{sigunguCode}` | 해당 다각형 안의 시설 ID·좌표·그룹·모든 공개 번역이 들어간 지도 응답 | 현재 1시간 Next 데이터 캐시를 우선 재사용하고 `region:{code}:markers` 태그 추가. 언어별로 원본 응답을 중복 저장하지 않음. 변경 시 이전·새 구·군을 모두 만료. |
| 지역 페이지 `page:{build}:{locale}:region:{code}` | 지도와 언어별 제목·문구·메타데이터 | 현재 1시간 ISR 유지. 영향받은 구·군의 6개 언어 경로만 만료. 전국/시·도는 집계가 변할 때만 만료. |
| 지역 수 `region-counts:{code}` | 공개 시설 수, 상위 시·도/전국 합계, 집계 시각 | 빌드 JSON 대신 공개 집계 API 또는 갱신 가능한 스냅샷. 1시간 보조 TTL + 지역별 변경 이벤트. 코드·수치만 저장하고 언어별 숫자 형식은 렌더링 시 적용. |
