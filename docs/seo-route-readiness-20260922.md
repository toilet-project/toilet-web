...
한국어와 외국어 시설 사이트맵은 API의 현재 이름·좌표로 경로를 만들기 때문에 웹 빌드에 포함된 옛 이름 스냅샷에 의존하지 않는다. 공개 URL은 `/sitemap-toilets-{shard}.xml` 및 `/sitemap-toilets-{shard}-{locale}.xml`이다. 운영 빌드에서 외국어 UI가 꺼져 있으면 외국어 상세가 404인 상태와 맞추어 외국어 사이트맵·hreflang도 출력하지 않는다. 기존 상세·별칭 경로와 같은 URL 생성기를 사용하고, 번역 변경은 기존 `catalogChanged` 무효화 이벤트와 1시간 원본 캐시 만료로 반영한다. API를 먼저 배포해야 웹이 새 사이트맵 원본을 읽을 수 있다.
