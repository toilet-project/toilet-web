...
;(async()=>{
 ...
 try {
  for(const width of [390,320,1280]) {
   ...
   await panel.getByLabel('시작일',{exact:true}).fill('2026-08-02');await panel.getByLabel('종료일',{exact:true}).fill('2026-08-02');await panel.getByRole('button',{name:'적용',exact:true}).click();await ready();await count(10)
   ...
   await panel.getByRole('button',{name:'날짜 직접 선택'}).click()
   await panel.getByLabel('시작일',{exact:true}).fill('2026-07-01');await panel.getByLabel('종료일',{exact:true}).fill('2026-07-01');await panel.getByRole('button',{name:'적용',exact:true}).click();await ready()
