...
const api = createServer((req,res) => {
  ...
  if (req.url === '/api/v1/toilets/900001') return res.end(JSON.stringify(fixture))
...
const env = {...process.env, NEXT_BUILD_DIR:'.next-smoke', TOILET_API_ORIGIN:`http://127.0.0.1:${apiPort}`, SITE_INDEXABLE:'false', NEXT_TELEMETRY_DISABLED:'1', NEXT_PUBLIC_KAKAO_JAVASCRIPT_KEY:'smoke-public-key'}
let server
let output = ''
const run = args => {
...
try {
  ...
  console.log(JSON.stringify({result:'PASS',ssr:true,canonical:true,noindex:true,cacheHits:3,upstreamRequestsForDetail:counts.get('/api/v1/toilets/900001'),real404Cases:4,missingCoordinates:true,upstreamFailureStatus:500},null,2))
} finally {
