...
import {buildIdFromKey,normalizeDeploymentStatus,planIncrementalCacheCleanup,sameActiveDeployment} from '../scripts/cache/cleanup-lib.mjs'
...
const releases=[
  {schema:1,workerVersion:old,buildId:'build-old',deployedAt:'2026-09-01T00:00:00.000Z'},
  {schema:1,workerVersion:previous,buildId:'build-previous',deployedAt:'2026-09-14T00:00:00.000Z'},
  {schema:1,workerVersion:active,buildId:'build-active',deployedAt:'2026-09-15T00:00:00.000Z'},
]
test('only recognized retired builds outside rollback protection are selected',()=>{
  const objects=[
    {key:'incremental-cache/build-active/a.cache',size:10},{key:'incremental-cache/build-previous/b.cache',size:20},
    {key:'incremental-cache/build-old/c.cache',size:30},{key:'public-toilets/v1/toilets/1.json',size:40},
  ...
  assert.deepEqual(plan.deleteObjects.map(row=>row.key),['incremental-cache/build-old/c.cache'])
...
test('missing active manifest and ambiguous keys fail closed',()=>{
  ...
  assert.equal(buildIdFromKey('incremental-cache/build/hash.cache'),'build');assert.equal(buildIdFromKey('other/build/hash.cache'),null)
})
