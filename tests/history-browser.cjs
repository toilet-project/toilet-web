...
;(async()=>{
 ...
 try {
  for(const width of [390,320]) {
   ...
   const bodyHeight=await shell.locator('.history-filter-body').evaluate(el=>el.offsetHeight)
   const before=await shell.locator('.history-filter-body').boundingBox()
   await shell.evaluate(el=>{el.scrollTop=40});await page.waitForTimeout(50)
   const midway=await shell.locator('.history-filter-body').boundingBox()
   assert.ok(Math.abs(before.y-midway.y-40)<=1,'filters move up with the scroll, not a height animation')
   assert.equal(await shell.locator('.history-filter-body').evaluate(el=>el.offsetHeight),bodyHeight,'scrolling preserves filter height and list layout')
   ...
   assert.equal(await shell.locator('.history-filter-disclosure').evaluate(el=>getComputedStyle(el).backgroundColor),'rgb(255, 255, 255)')
   ...
   const reopened=await shell.locator('.history-filter-body').boundingBox(), readingPosition=await shell.evaluate(el=>el.scrollTop)
   await shell.evaluate(el=>{el.scrollTop+=30});await page.waitForTimeout(50)
   const moved=await shell.locator('.history-filter-body').boundingBox()
   assert.ok(Math.abs(reopened.y-moved.y-30)<=1,'reopened filters also follow the next scroll naturally')
   assert.ok(await shell.evaluate(el=>el.scrollTop)>=readingPosition,'reopening filters does not reset the reading position')
   await shell.locator('.history-filter-disclosure').click()
   assert.equal(await shell.locator('.history-filter-body').evaluate(el=>el.hidden),true,'explicit collapse hides the controls')
   await ensureFilters()
