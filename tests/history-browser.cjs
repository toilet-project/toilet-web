...
;(async()=>{
 ...
 try {
  for(const width of [390,320]) {
   ...
   const collapsedFilter=await shell.locator('.history-filter-shell').boundingBox()
   assert.ok(collapsedFilter.y>=pinnedHeading.y+pinnedHeading.height-1,'collapsed filter stays below the fixed title row')
   assert.equal(await shell.locator('.history-filter-body').getAttribute('aria-hidden'),'true','collapsed filter body leaves the visible layout')
   ...
   await shell.getByRole('button',{name:'날짜 직접 선택'}).click();await shell.getByLabel('시작일',{exact:true}).fill('2026-09-10');await shell.getByLabel('종료일',{exact:true}).fill('2026-09-01');await shell.getByRole('button',{name:'적용',exact:true}).click();await shell.getByRole('alert').filter({hasText:'종료일'}).waitFor();assert.equal(await shell.locator('.my-report-item').count(),1);await shell.getByRole('button',{name:'날짜 직접 선택'}).click()
   ...
   if(reviewCount>10)await shell.getByRole('button',{name:'최근 30일',exact:true}).click()
   ...
   await ensureFilters()
   await shell.getByRole('button',{name:'날짜 직접 선택'}).click();await shell.getByLabel('시작일',{exact:true}).fill('2026-08-02');await shell.getByLabel('종료일',{exact:true}).fill('2026-08-02');await shell.getByRole('button',{name:'적용',exact:true}).click();await shell.getByText('이 기간에 남긴 리뷰가 없어요',{exact:true}).waitFor()
