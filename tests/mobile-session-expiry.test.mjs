...
test('private panels remount per owner and late notification counters are guarded', () => {
  ...
  assert.ok(app.includes('owner={authProfile?.userId ?? null}'))
  const menu = source('components/NotificationMenu.tsx')
  assert.ok(menu.includes('<NotificationMenuFeed key={owner}'))
  assert.match(menu, /if \(!active.current\) return/)
...
test('expired session clears private details and retains only a navigation intent', () => {
  ...
  assert.match(app, /if \(openInbox\) \{ if \(isDesktop\) setIsNotificationsOpen\(true\); else setMobileTab\('notifications'\); return \}/)
})
