...
test('review touch controls use shared stars, direct waiting time and an accessible refreshing status', () => {
  ...
  assert.match(form, /aria-label=\{t\('review.refreshLocation'\)\}/)
...
test('review checks stay in the card button; only success opens the editor and retries retain drafts', () => {
  ...
  assert.match(open, /t\('review.mobileRequired'\)/)
  ...
  assert.match(apiHook, /t\('review.mobileRequired'\)/)
  assert.match(apiHook, /title=\{t\('review.existingTitle'\)\}/)
  assert.match(apiHook, />\{t\('common.back'\)\}<\/button>/)
  assert.match(apiHook, />\{t\('review.viewMine'\)\}<\/button>/)
