...
module.exports = async function runAccountChecks(browser, origin) {
  ...
  for (const width of [320, 390, 1280]) {
    ...
    try {
      ...
      assert.equal(writes.length, 0, 'opening policies or cancelling closure does not consent or mutate')
      await page.getByRole('button', { name: 'Close account', exact: true }).click()
      await page.waitForFunction(() => !document.querySelector('.withdrawal-choice input').disabled)
      assert.equal(await retention.isChecked(), false)
      await page.screenshot({ path: path.resolve(`.next/i18n-account-${width}.png`) })
      await page.getByRole('button', { name: 'Close and request erasure', exact: true }).click()
      await page.getByRole('dialog', { name: 'Account closure result' }).getByText(/still pending/).waitFor()
      assert.deepEqual(writes.at(-1), { kind: 'withdraw', body: { retainForRecovery: false } })
      await page.goto(origin + '/en'); await openAccount()
      await page.getByRole('button', { name: 'Close account', exact: true }).click()
      await page.waitForFunction(() => !document.querySelector('.withdrawal-choice input').disabled)
      await retention.check(); await page.getByRole('button', { name: 'Close and retain recovery data', exact: true }).click()
      await page.getByRole('dialog', { name: 'Account closure result' }).getByText(/20\/12\/2026/).waitFor()
