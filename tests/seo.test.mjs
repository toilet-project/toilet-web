...
test('production allows toilets; preview blocks crawling without advertising sitemap',()=>{
  ...
  const blocked=production.rules.find(rule=>Array.isArray(rule.userAgent))
  const publicRules=production.rules.find(rule=>rule.userAgent==='*')
  assert.deepEqual(blocked.userAgent,['ClaudeBot','Moltbot','GPTBot','Google-Extended','FacebookBot','Meta-ExternalAgent'])
  assert.equal(blocked.disallow,'/')
  assert.equal(publicRules.allow,'/')
  assert.ok(!publicRules.disallow.some(path=>'/toilet/1'.startsWith(path)))
  assert.ok(!blocked.userAgent.includes('ChatGPT-User'))
  assert.ok(!blocked.userAgent.includes('OAI-SearchBot'))
  assert.ok(!blocked.userAgent.includes('Claude-SearchBot'))
