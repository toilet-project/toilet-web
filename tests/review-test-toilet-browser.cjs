...
;(async () => {
  ...
  try {
    for (const width of [390, 320, 1280]) {
      ...
      await openReview()
      await form.getByRole('alert').getByText(/150m 이내/).waitFor()
      assert.equal(await form.locator('input, textarea').count(), 0)
      assert.equal(await form.getByRole('button', { name: '리뷰 남기기', exact: true }).count(), 0)
