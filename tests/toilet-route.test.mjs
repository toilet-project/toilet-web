...
test('loading and first HTML retain current card rows and mobile geometry without inventing data', async () => {
  ...
  assert.match(app, /<LoadingOpenTime \/><ToiletCommunityRow pendingReport=\{Boolean\(onReport\)\} \/><DetailLoadingFields inline \/>/)
  ...
  assert.match(bridge, /<ToiletCommunityRow pendingReport \/>/)
