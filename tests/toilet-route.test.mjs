...
test('interactive detail fetch does not wait for the route; initial SSR still seeds the cache', async () => {
  ...
  assert.match(app,/if \(!disposed && !snapshot && !initialRouteRef\.current\.detail && \(!resume \|\| usedFallback\) && !testToilet\)/)
})
...
test('unknown detail values use placeholders and collapsed mobile errors remain visible', async () => {
  ...
  assert.match(app,/toiletTypeLabel\(displayToiletDetail\?\.toiletType \|\| displaySelectedToilet.toiletType, locale\)/)
  assert.match(app,/!displayToiletDetail && isDetailLoading && <DetailLoadingFields/)
  assert.match(loading,/role="status" aria-label=\{t\('detail.loading'\)\}/)
...
test('loading and first HTML retain current card rows and mobile geometry without inventing data', async () => {
  ...
  assert.match(bridge, /aria-label=\{t\('map.distanceLoading'\)\}/)
  const { message } = await import('../src/i18n/messages.ts')
  assert.equal(message('ko', 'map.distanceLoading'), '거리 계산 중')
