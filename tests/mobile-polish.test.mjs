...
test('review summaries follow the review action and divider, before addresses, while staying out of the collapsed mobile card', async () => {
  ...
  assert.match(css, /\.public-review-full-header\s*\{[^}]*padding: 56px 18px 16px/)
  assert.match(css, /\.public-review-full-header h2\s*\{[^}]*font-size: 19px[^}]*font-weight: 700/)
  assert.match(css, /\.public-review-back\s*\{[^}]*position: absolute[^}]*left: 2px[^}]*width: 44px[^}]*height: 44px/)
  assert.doesNotMatch(css, /\.public-review-full-header\s*\{[^}]*border-bottom/)
  assert.match(css, /\.public-review-full-reviews\s*\{[^}]*padding: 12px 12px 4px[^}]*border: 1px solid #e2e9e4[^}]*border-radius: 14px/)
  assert.match(css, /\.place-card\.is-review-list-open > \.close-button\s*\{[^}]*top: 2px[^}]*right: 2px[^}]*width: 44px[^}]*height: 44px/)
