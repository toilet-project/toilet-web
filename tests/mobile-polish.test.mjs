...
test('review summaries follow the review action and divider, before addresses, while staying out of the collapsed mobile card', async () => {
  ...
  assert.match(css, /\.public-reviews\s*\{[^}]*border: 1px solid #e2e9e4[^}]*border-radius: 14px/)
  assert.match(css, /\.public-review-rating\s*\{[^}]*background: transparent/)
  assert.match(css, /\.public-review-row\s*\{[^}]*border-bottom: 1px solid #eef2ef/)
  assert.match(css, /\.public-review-row:last-child\s*\{[^}]*border-bottom: 0/)
  assert.doesNotMatch(css, /\.public-review-row\s*\{[^}]*border-radius/)
