...
test('review summaries follow the review action and divider, before addresses, while staying out of the collapsed mobile card', async () => {
  ...
  assert.match(css, /\.public-reviews\s*\{[^}]*position: relative[^}]*padding: 12px 12px 16px[^}]*border: 1px solid #e2e9e4[^}]*border-radius: 14px/)
  ...
  assert.match(css, /\.public-review-full-reviews\s*\{[^}]*overflow: hidden[^}]*border: 1px solid #e2e9e4[^}]*border-radius: 14px/)
  assert.match(css, /\.public-review-full-list\s*\{[^}]*overflow-y: auto[^}]*padding: 12px 12px 16px/)
