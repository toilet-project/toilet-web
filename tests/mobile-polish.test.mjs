...
test('future toilet metrics are placeholders in a 44px row, with a labeled report action', async () => {
  ...
  assert.match(css, /\.report-entry-button\.report-icon-button\s*\{[^}]*flex: 0 0 44px[^}]*margin: 0 0 0 auto/)
  assert.match(css, /\.toilet-community-row\s*\{[^}]*container-type: inline-size/)
  assert.match(css, /@container \(max-width: 240px\)[\s\S]*padding-right: 4px[\s\S]*padding-left: 4px/)
