...
test('cache workflows stay manually dispatched and opt-in',()=>{
  ...
  assert.match(prewarm,/checkpoint_source_run_id/)
  assert.match(prewarm,/fail-on-cache-miss: true/)
  assert.match(prewarm,/Exact checkpoint source run ID is required/)
  ...
  assert.match(prewarm,/--failed-from-checkpoint/)
