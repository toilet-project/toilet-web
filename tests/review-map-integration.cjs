...
;(async () => {
  ...
  try {
    for (const width of [390, 320, 1280]) {
      ...
      assert.equal(await form.getByRole('button',{name:'리뷰 남기기',exact:true}).count(),0,'check opens immediately without inviting an ineligible draft')
      assert.equal(await form.locator('input, textarea').count(),0,'no inputs before verification')
      assert.equal(await form.getByRole('button',{name:'위치 새로고침',exact:true}).isDisabled(),true)
      assert.equal(await form.getByRole('button',{name:'위치 새로고침',exact:true}).innerText(),'')
      assert.equal(await form.locator('.rv-eligibility').evaluate(el=>getComputedStyle(el,'::before').animationName),'rv-location-pulse')
      await page.emulateMedia({ reducedMotion: 'reduce' })
      assert.equal(await form.locator('.rv-eligibility').evaluate(el=>getComputedStyle(el,'::before').animationName),'none')
      ...
      assert.equal(await form.locator('input, textarea').count(),0,'GPS alone cannot open the editor')
      ...
      if(width<600) await myReviews.getByRole('button',{name:'내 페이지로 돌아가기'}).click()
      ...
      for (const [change, expected] of [[{accuracy:51},'50m'],[{accuracy:10,age:300001},'5분'],[{age:0,latitude:0},'150m'],[{latitude:groupDetail.latitude,denied:true},'위치 권한']]) {
        ...
        assert.equal(await blocked.locator('input, textarea').count(),0,'failed initial verification never offers review inputs')
        assert.equal(await blocked.getByRole('button',{name:'리뷰 남기기',exact:true}).count(),0)
        if(expected==='150m') {
          const line = await blocked.locator('.rv-eligibility>span').evaluate(el=>({fits:el.scrollWidth<=el.clientWidth,height:el.getBoundingClientRect().height,lineHeight:parseFloat(getComputedStyle(el).lineHeight)}))
          assert.ok(line.fits && line.height <= line.lineHeight+1,'150m message fits one line without truncation')
        ...
        if(expected==='위치 권한') {
          ...
          await blocked.getByRole('button',{name:'위치 새로고침',exact:true}).click()
          await page.waitForFunction(()=>typeof window.reviewReleaseGeo==='function')
          assert.equal(await blocked.locator('.rv-eligibility').evaluate(el=>getComputedStyle(el,'::before').animationName),'rv-location-pulse')
          assert.equal(await blocked.getByRole('button',{name:'위치 새로고침',exact:true}).isDisabled(),true)
          await page.evaluate(()=>{window.reviewReleaseGeo();window.reviewTestFix.hold=false;delete window.reviewReleaseGeo})
          await page.waitForFunction(()=>document.querySelector('.rv-dialog-footer .rv-primary')?.disabled===false)
      ...
      await context.close()
      console.log(`PASS actual map ${width}; precheck visible ${openMs}ms; 4-minute fix accepted and older than 5 minutes rejected; inputs only after auth+GPS; blue/red paper; later retry retains draft; create/edit/detach; no business writes`)
    }
