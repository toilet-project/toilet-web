...
;(async () => {
  ...
  try {
    for (const width of [390, 320, 1280]) {
      ...
      assert.equal(await expanded.locator('.coordinate-opening-row').getByRole('button',{name:'정보 제공하기'}).count(),1)
      assert.equal(await expanded.locator('.coordinate-opening-row .review-card-report').innerText(),'','group report uses only a siren icon')
