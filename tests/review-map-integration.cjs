  ...
  try {
    for (const width of [390, 320, 1280]) {
      ...
      for (const [change, expected] of [[{accuracy:51},'50m'],[{accuracy:10,age:300001},'5분'],[{age:0,latitude:0},'150m'],[{latitude:groupDetail.latitude,denied:true},'위치 권한']]) {
        ...
        const toast = page.locator('.location-message').filter({hasText:expected})
        await toast.waitFor()
        assert.equal(await toast.evaluate(el=>{const r=el.getBoundingClientRect();return document.elementFromPoint(r.x+r.width/2,r.y+r.height/2)===el}),true,'toast is not hidden behind the group card')
        ...
        if(expected==='150m') {
          const line = await toast.evaluate(el=>{const range=document.createRange();range.selectNodeContents(el);return {width:range.getBoundingClientRect().width,height:range.getBoundingClientRect().height,available:el.clientWidth-24,lineHeight:parseFloat(getComputedStyle(el).lineHeight)}})
