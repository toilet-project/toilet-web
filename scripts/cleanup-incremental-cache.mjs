...
import {assertReviewedCleanupPlan,normalizeDeploymentStatus,planIncrementalCacheCleanup,sameActiveDeployment} from './cache/cleanup-lib.mjs'
...
if(args.execute){
  ...
  await writeFile(args.report,JSON.stringify(result,null,2)+'\n')
  try{
    await store.delete(plan.deleteObjects.map(object=>object.key),{onBatch:({deletedFiles,totalFiles})=>{
