...
export type PhotoState = { available: boolean; publicPhoto: boolean; imageVersion: string | null }
export function decodePhoto(value: unknown): PhotoState {
  if (!value || typeof value !== 'object') throw new Error('사진 설정을 확인하지 못했어요.')
  const v = value as Record<string, unknown>
  if (typeof v.available !== 'boolean' || typeof v.publicPhoto !== 'boolean'
    || !(v.imageVersion === null || typeof v.imageVersion === 'string' && /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(v.imageVersion))
    || v.publicPhoto && v.imageVersion === null) throw new Error('사진 설정을 확인하지 못했어요.')
  return { available: v.available, publicPhoto: v.publicPhoto, imageVersion: v.imageVersion }
}
