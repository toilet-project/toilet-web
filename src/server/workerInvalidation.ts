...
export async function persistWorkerInvalidation(ids: number[], catalogChanged = false, districtCodes: string[] | null = null) {
  ...
  if (ids.length && districtCodes === null) tags.push({tag:'region-markers', stale:now, expire:now})
  else for (const code of districtCodes ?? []) tags.push({tag:`region-markers:${code}`, stale:now, expire:now})
