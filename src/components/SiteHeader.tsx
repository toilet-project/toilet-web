...
export function SiteHeader({ path, languagePaths }: { path: string; languagePaths?: Partial<Record<Locale, string>> }) {
  ...
  return <>
    ...
    <nav className="mobile-navigation site-mobile-nav" aria-label={t('nav.main')}>
      ...
      <Link href={regions} prefetch={!path.startsWith('/regions')} aria-current={path.startsWith('/regions') ? 'page' : undefined}><span className="mobile-nav-icon"><BottomIcon name="regions" /></span><span>{t('nav.community')}</span></Link>
