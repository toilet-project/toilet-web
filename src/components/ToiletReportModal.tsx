...
type ReportType = 'choice' | 'location' | 'openTime' | 'complete'
...
export function ToiletReportModal({ toilet, latitude, longitude, onClose }: { toilet: ToiletDetailResponse; latitude: number; longitude: number; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    if (step === 'location' && (!roadAddress || !isLocationConfirmed)) { setError('표시된 위치와 도로명 주소를 확인해 주세요.'); return }
    if (step === 'openTime' && !openTime.trim()) { setError('변경할 개방 시간을 입력해 주세요.'); return }

    setIsSubmitting(true)
    try {
      await createToiletReport(step === 'location'
  ...
  return <div className="report-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="report-modal" role="dialog" aria-modal="true" aria-labelledby="report-modal-title">
      ...
      {step === 'location' && <>
        <button type="button" className="report-back" onClick={() => setStep('choice')}>← 유형 선택</button>
        <span className="report-modal-eyebrow">위치 제보</span>
        <h1 id="report-modal-title">지도를 움직여 핀을 맞춰 주세요</h1>
        <p className="report-modal-description">가운데 고정 핀이 실제 화장실 위치를 가리키도록 지도를 움직여 주세요.</p>
        ...
        <label className="report-confirmation"><input type="checkbox" checked={isLocationConfirmed} onChange={(event) => setIsLocationConfirmed(event.target.checked)} /> 이 위치와 주소가 맞습니다.</label>
        ...
        <button type="button" className="report-submit" disabled={isSubmitting || isAddressLoading} onClick={() => void submit()}>{isSubmitting ? '접수 중…' : '위치 제보 접수'}</button>
      </>}
      {step === 'openTime' && <>
        <button type="button" className="report-back" onClick={() => setStep('choice')}>← 유형 선택</button>
