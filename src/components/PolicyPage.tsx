...
export function PolicyPage({ kind, embedded = false }: { kind: PolicyPageKind; embedded?: boolean }) {
  ...
  if (kind === 'privacy') return <PolicyLayout title="개인정보 처리방침" embedded={embedded}>
    ...
    <section><h2>3. 보유 기간</h2><ul><li>이메일·인증 여부·일반 로그인 정보: 회원 탈퇴 시 삭제·초기화</li><li>계정 복구용 정보: 탈퇴 시 별도 선택 동의한 경우에 한하여 탈퇴 시각부터 달력상 3개월. 소셜 제공자·보호된 고유 식별자, 닉네임, 회원·제보 연결 정보와 동의·탈퇴·삭제 예정 시각을 복구 목적으로 보관합니다. 이메일·소셜 토큰은 복구용으로 보관하지 않습니다.</li><li>복구용 보관 미동의 또는 보관 중 삭제 요청: 회원·소셜 식별정보, 회원별 동의·알림 및 제보의 회원 연결·자유 입력 사유를 파기합니다. 같은 소셜 로그인으로 재가입해도 이전 계정과 연결되지 않습니다.</li><li>리프레시 토큰: 발급 후 최대 14일 또는 로그아웃·탈퇴 시까지</li><li>제보 및 처리 이력: 처리 완료 후 3년. 회원정보 파기 시 작성자 연결과 개인정보가 포함될 수 있는 자유 입력 사유·검토 메모를 제거합니다. 승인되어 공공 화장실 정보에 반영된 내용은 유지합니다.</li><li>운영 감사 로그: 처리 완료 후 3년. 회원정보 파기 시 해당 회원 식별 연결과 상세 내용을 제거합니다.</li><li>일반 접속 로그: 최대 3개월</li></ul><p>복구용 보관 동의는 선택 사항이며 거부해도 탈퇴할 수 있습니다. 보관 중에는 동일 소셜 로그인 후 복구하지 않고 즉시 삭제를 요청하거나 개인정보 문의 이메일을 이용할 수 있습니다. 보유 목적이 끝난 정보는 복구하기 어려운 방법으로 파기하며, 관계 법령에서 별도 보존을 요구하는 경우에는 근거와 기간에 따라 분리 보관합니다. 장애로 파기가 지연되면 서비스 이용·복구를 차단한 상태로 재시도합니다.</p></section>
