...
export function PolicyPage({ kind, embedded = false }: { kind: PolicyPageKind; embedded?: boolean }) {
  ...
  if (kind === 'privacy') return <PolicyLayout title="개인정보 처리방침" embedded={embedded}>
    ...
    <section id="analytics"><SectionHeading>4-1. 서비스 이용 통계</SectionHeading><p>서비스 운영 상태와 이용 불편을 확인하기 위해 페이지 유형, 유입 분류, 기기 환경과 주요 기능 사용 여부를 급똥 서버에서 집계합니다. 분석용 Google 태그와 Google Analytics 쿠키는 사용하지 않습니다.</p><dl className="policy-storage-facts"><div><dt>처리 목적</dt><dd>방문·조회·유입·기기 환경·주요 기능 사용의 통계 작성과 서비스 개선</dd></div><div><dt>식별 최소화</dt><dd>접속망 일부와 기기 정보를 기간별 암호화 해시로 바꿔 중복 방문을 계산하며 원본 IP를 분석 테이블에 저장하지 않습니다.</dd></div><div><dt>보유 기간</dt><dd>개별 분석 이벤트는 최대 35일 뒤 삭제하고, 개인·원문 입력을 포함하지 않는 일별 집계 통계는 장기 운영 추이 확인을 위해 보관합니다.</dd></div><div><dt>수집 제외</dt><dd>원문 검색어, 회원·소셜 식별값, 정확한 GPS와 화장실 좌표·주소, 제보·리뷰 자유 입력 내용</dd></div></dl></section>
