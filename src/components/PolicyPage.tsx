...
export function PolicyPage({ kind, embedded = false }: { kind: PolicyPageKind; embedded?: boolean }) {
  ...
  if (kind === 'privacy') return <PolicyLayout title="개인정보 처리방침" embedded={embedded}>
    <section id="collection"><h2>1. 수집하는 개인정보</h2><ul><li>소셜 로그인: 제공자, 제공자 회원 식별값의 해시, 표시 이름, 이메일, 이메일 인증 여부</li><li>프로필 사진: 신규 가입 시 제공에 동의한 카카오 프로필 사진 또는 이용자가 직접 올린 사진을 변환한 이미지, 공개 범위, 수집 경로와 적용한 안내 버전·시각. 원본 파일과 카카오 원본 주소는 변환 후 저장하지 않습니다.</li><li>서비스 이용: 역할, 동의한 정책과 버전·시각, 최근 로그인 시각</li><li>정보 제보: 대상 화장실, 제보 내용, 선택한 좌표와 도로명 주소, 처리 상태와 관리자 메모</li><li>리뷰: 대상 화장실, 만족도·청결도·화장지 유무·대기시간, 선택 작성글, 작성·수정 시각, 작성자 계정 연결과 변경 방지용 리뷰 식별자</li><li>리뷰 작성 자격 확인: 요청 당시 현재 위치·위치 정확도·측정 시각. 화장실 150m 이내, 정확도 50m 이하, 최근 5분 이내인지 서버에서 확인하며 정확한 현재 위치 좌표를 리뷰에 저장하지 않습니다.</li><li>운영·보안: 접속 기록, 감사 로그, 오류 기록</li></ul></section>
