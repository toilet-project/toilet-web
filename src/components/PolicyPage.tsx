...
export function PolicyPage({ kind, embedded = false }: { kind: PolicyPageKind; embedded?: boolean }) {
  ...
  if (kind === 'terms') return <PolicyLayout title="서비스 이용약관" embedded={embedded}>
    ...
    <section><h2>3. 계정과 소셜 로그인</h2><p>Google 또는 Kakao 계정으로 본인 인증 후 필수 약관에 동의하면 계정이 활성화됩니다. 이용자는 자신의 계정을 안전하게 관리해야 하며 타인의 계정을 이용해서는 안 됩니다. 카카오에서 프로필 사진 제공에 동의하거나 직접 사진을 등록하면 공개 리뷰의 작성자 사진으로 표시되며, 내 페이지에서 언제든 공개를 끌 수 있습니다.</p></section>
    <section><h2>4. 제보·리뷰와 콘텐츠</h2>
  ...
  if (kind === 'privacy') return <PolicyLayout title="개인정보 처리방침" embedded={embedded}>
    <section id="collection"><h2>1. 수집하는 개인정보</h2><ul><li>소셜 로그인: 제공자, 제공자 회원 식별값의 해시, 표시 이름, 이메일, 이메일 인증 여부</li><li>프로필 사진: 신규 가입 시 제공에 동의한 카카오 프로필 사진 또는 이용자가 직접 올린 사진을 변환한 이미지, 공개 범위, 수집 경로와 적용한 안내 버전·시각. 등록된 사진은 공개 리뷰 작성자 사진으로 표시되며 내 페이지에서 언제든 공개를 끌 수 있습니다. 원본 파일과 카카오 원본 주소는 변환 후 저장하지 않습니다.</li><li>서비스 이용: 역할, 동의한 정책과 버전·시각, 최근 로그인 시각</li><li>정보 제보: 대상 화장실, 제보 내용, 선택한 좌표와 도로명 주소, 처리 상태와 관리자 메모</li><li>리뷰: 대상 화장실, 만족도·청결도·화장지 유무·대기시간, 선택 작성글, 작성·수정 시각, 작성자 계정 연결과 변경 방지용 리뷰 식별자</li><li>리뷰 작성 자격 확인: 요청 당시 현재 위치·위치 정확도·측정 시각. 화장실 150m 이내, 정확도 50m 이하, 최근 5분 이내인지 서버에서 확인하며 정확한 현재 위치 좌표를 리뷰에 저장하지 않습니다.</li><li>운영·보안: 접속 기록, 감사 로그, 오류 기록</li></ul></section>
    ...
    <section><h2>5. 이용자의 권리</h2><p>이용자는 자신의 제보 처리 상태와 내 리뷰를 확인하고, 작성 후 7일 이내에는 리뷰를 직접 수정하거나 작성자 정보 연결을 해제할 수 있으며 계정 화면에서 탈퇴할 수 있습니다. 직접 처리 기간이 지난 리뷰 또는 리뷰 글에 포함된 개인정보의 열람·정정·삭제·처리정지 요청은 리뷰를 식별할 수 있는 정보와 요청 부분을 문의 이메일로 접수할 수 있습니다.</p></section>
  ...
  return <PolicyLayout title="위치정보 이용 안내" embedded={embedded}>
    <section><h2>1. 언제 위치를 사용하나요?</h2><p>현재 위치 버튼을 누르거나 첫 방문 시 브라우저가 위치 권한을 요청할 때, 주변 화장실 조회와 직선거리 계산에 사용합니다. 로그인 사용자가 리뷰를 작성하려 할 때에는 화장실과 가까운지 확인하기 위해 최근 위치를 다시 사용합니다.</p></section>
    <section><h2>2. 서버에 저장하나요?</h2><p>일반 지도 조회에 사용한 기기 GPS 좌표는 급똥 서버에 저장하지 않고 브라우저 안에서 지도 중심과 거리 계산에만 사용합니다. 리뷰 작성 시에는 현재 위치·정확도·측정 시각을 서버로 전송해 대상 화장실 150m 이내, 정확도 50m 이하, 최근 5분 이내인지 확인하지만 정확한 현재 위치 좌표를 리뷰에 저장하지 않습니다.</p></section>
