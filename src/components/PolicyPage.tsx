...
export function PolicyPage({ kind, embedded = false }: { kind: PolicyPageKind; embedded?: boolean }) {
  ...
  if (kind === 'privacy') return <PolicyLayout title="개인정보 처리방침" embedded={embedded}>
    <section id="collection"><SectionHeading>1. 수집하는 개인정보</SectionHeading><ul><li>소셜 로그인: 제공자, 제공자 회원 식별값의 해시, 표시 이름, 이메일, 이메일 인증 여부</li><li>프로필 사진: 신규 가입 시 제공에 동의한 카카오 프로필 사진 또는 이용자가 직접 올린 사진을 변환한 이미지, 공개 범위, 수집 경로와 적용한 안내 버전·시각. 등록된 사진은 공개 리뷰 작성자 사진으로 표시되며 내 페이지에서 언제든 공개를 끌 수 있습니다. 원본 파일과 카카오 원본 주소는 변환 후 저장하지 않습니다.</li><li>서비스 이용: 역할, 동의한 정책과 버전·시각, 최근 로그인 시각</li><li>정보 제보: 대상 화장실, 제보 내용, 선택한 좌표와 도로명 주소, 처리 상태와 관리자 메모</li><li>리뷰: 대상 화장실, 만족도·청결도·화장지 유무·대기시간, 선택 작성글, 작성·수정 시각, 작성자 계정 연결과 변경 방지용 리뷰 식별자</li><li>리뷰 작성 자격 확인: 요청 당시 현재 위치·위치 정확도·측정 시각. 화장실 150m 이내, 정확도 50m 이하, 최근 5분 이내인지 서버에서 확인하며 정확한 현재 위치 좌표를 리뷰에 저장하지 않습니다.</li><li>선택 분석: 분석 사용을 허용한 경우 페이지 유형, 방문·조회·세션·참여 시간, 유입 경로, 기기·운영체제·브라우저, 국가·도시 수준의 접속 지역, 검색 성공 여부와 결과 수 구간, 마커·상세·제보·리뷰 등 기능 사용 이벤트. 원문 검색어, 이메일·계정 식별값, 정확한 위치·좌표, 화장실 주소와 자유 입력 내용은 분석 도구로 보내지 않습니다.</li><li>운영·보안: 접속 기록, 감사 로그, 오류 기록</li></ul></section>
    <section><SectionHeading>2. 이용 목적</SectionHeading><p>회원 식별, 소셜 로그인, 제보 접수·처리 결과 제공, 가까운 현장에서의 리뷰 작성 자격 확인, 리뷰 제공과 부정·중복 이용 방지, 장애 대응, 서비스 품질 개선에 사용합니다. 선택 분석 정보는 이용 규모와 주요 기능·페이지·유입·기기 환경의 변화를 집계해 장애와 이용 불편을 찾는 데 사용합니다.</p></section>
    ...
    <section><SectionHeading>4. 외부 서비스 이용</SectionHeading><p>로그인을 위해 Google·Kakao OAuth, 지도와 주소 확인을 위해 Kakao Maps, 웹 제공·보안·이메일 전달과 프로필 사진 보관을 위해 Cloudflare를 이용합니다. 분석을 허용한 경우에는 Google Analytics 4를 이용해 위에서 안내한 이용 정보를 집계합니다. 각 제공자가 인증과 전송 과정에서 처리하는 정보에는 해당 제공자의 정책이 적용됩니다.</p></section>
    <section id="analytics"><SectionHeading>4-1. Google Analytics 선택 분석</SectionHeading><p>분석은 기본적으로 꺼져 있으며 화면에서 ‘허용’을 선택한 뒤에만 Google 태그를 불러옵니다. 거부해도 지도·검색·로그인·제보·리뷰를 포함한 서비스 기능을 그대로 이용할 수 있습니다. 분석 설정에서 언제든 사용을 중지할 수 있고, 중지하면 이후 전송을 막고 브라우저에서 확인 가능한 Google Analytics 쿠키 삭제를 시도합니다.</p><dl className="policy-storage-facts"><div><dt>처리 제공자</dt><dd>Google LLC (Google Analytics 4)</dd></div><div><dt>처리 목적</dt><dd>방문·조회·유입·기기 환경·주요 기능 사용의 통계 작성과 서비스 개선</dd></div><div><dt>보유 설정</dt><dd>이벤트 수준 데이터는 GA4 속성에서 2개월 보유로 설정하며, 집계 보고서는 Google Analytics의 정책과 속성 설정에 따라 처리합니다.</dd></div><div><dt>전송 제외</dt><dd>원문 검색어, 회원·소셜 식별값, 정확한 GPS와 화장실 좌표·주소, 제보·리뷰 자유 입력 내용</dd></div></dl></section>
...
export function PolicyFooter() {
  return <div className="policy-footer"><nav aria-label="서비스 정책"><a href="/policies/terms">이용약관</a><a href="/policies/privacy">개인정보 처리방침</a><a href="/policies/location">위치정보 안내</a><AnalyticsSettingsButton /><a href="mailto:privacy@geupddong.com">문의</a></nav><small>© 2026 급똥</small></div>
}
