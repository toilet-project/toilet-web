...
export function ReviewPreview() {
  ...
  return <div className="rv-preview">
    ...
    <main className="rv-preview-main">
      ...
      {!!reviews.filter(r=>r.authorRemoved).length&&view==='map'&&<section className="rv-retained"><h2>작성자 정보를 지운 체험 리뷰</h2>{reviews.filter(r=>r.authorRemoved).map(r=><article key={r.id}><strong>익명</strong><span>★ {r.satisfaction}.0 · 청결 {r.cleanliness}.0 · 휴지 {r.paper?'있음':'없음'}</span><p>{r.comment||'내용 없음'}</p></article>)}</section>}
    </main>
