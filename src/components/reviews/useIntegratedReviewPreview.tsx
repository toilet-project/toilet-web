...
type MineNavigation = { embedded: boolean; onOpen: () => void; onClose: () => void }
...
export function useIntegratedReviewPreview(owner: string | null, access: Access, mineNavigation?: MineNavigation) {
  ...
  const close = () => { request.current++; setChecking(false); setTarget(null); setEditing(null); setMine(false); setSaved(false); setMessage('') }
  ...
  return { open, openMine, close, summary, modal, page: REVIEW_DESIGN_PREVIEW && mine && mineNavigation?.embedded ? mineContent : null, active: Boolean(target || mine || saved || checking) }
}
