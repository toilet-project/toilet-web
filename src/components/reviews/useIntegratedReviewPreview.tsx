...
type Access = { requireLogin: () => void; notify: (message: string) => void; verifySession: () => Promise<boolean> }
...
export function useIntegratedReviewPreview(owner: string | null, access: Access) {
  ...
  const session = async () => {
    ...
    try { valid = await Promise.race([access.verifySession(), new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error('Session check timed out')), 12_000) })]) }
  ...
  const open = async (next: Target) => {
    ...
    try {
      ...
      await requireReviewLocation(next)
      ...
      if (token === request.current) access.notify(error instanceof ReviewGateError ? error.message : '위치를 확인하지 못했어요. 다시 시도해 주세요.')
  ...
  const openMine = async () => {
    ...
    } catch (error) { if (token === request.current) access.notify(error instanceof ReviewGateError ? error.message : '로그인을 확인해 주세요.') }
    finally { if (token === request.current) setChecking(false) }
  }
  ...
  const save = async (value: ReviewInput) => {
    ...
    const token = ++request.current
    // Re-measure for a new review: a form left open does not extend the one-minute limit.
    // Editing a previously verified review only requires the same signed-in author within seven days.
    const measuredAt = !editing ? await requireReviewLocation(target) : null
    await session()
  ...
  const modal = !REVIEW_DESIGN_PREVIEW ? null : target ? <ReviewDialog key={`${owner}:${editing?.id ?? target.id}`} toiletName={target.name} initial={editing ?? undefined} onClose={close} onSave={save} />
  ...
  return { open, openMine, close, summary, modal: <>{modal}{REVIEW_DESIGN_PREVIEW && checking && <div className="rv-location-check" role="status"><span>리뷰 이용 조건을 확인하고 있어요…</span><button type="button" onClick={() => { request.current++; setChecking(false) }}>취소</button></div>}</>, active: Boolean(target || mine || saved || checking) }
}
