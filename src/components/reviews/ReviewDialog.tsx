...
export function ReviewDialog({ toiletName, initial, onClose, onSave, eligibility, onRetryEligibility }: { toiletName: string; initial?: ReviewInput; onClose: () => void; onSave: (value: ReviewInput) => Promise<void>; eligibility?: ReviewEligibility; onRetryEligibility?: () => void }) {
  ...
  const [started, setStarted] = useState(!eligibility || eligibility.status === 'ready')
  // Initial checks happen before input. Once edited, later checks retain the draft.
  const canWrite = started || !eligibility || eligibility.status === 'ready'
  const busy = useRef(false), active = useRef(false)
  useEffect(() => { active.current = true; return () => { active.current = false } }, [])
  const change = <K extends keyof ReviewInput>(key: K, next: ReviewInput[K]) => { setStarted(true); setError(''); setValue(current => ({...current, [key]: next})) }
