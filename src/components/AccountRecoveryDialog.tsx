...
export function AccountRecoveryDialog() {
  ...
  const decide = async (action: 'RESTORE' | 'ERASE') => {
    ...
    try {
      ...
      else setFinished(result.erasurePending ? '삭제 요청을 접수했어요. 복구는 중지됐으며 파기 작업을 재시도합니다.' : '회원정보를 파기했어요. 같은 계정으로 가입해도 이전 제보는 연결되지 않습니다.')
  ...
  return <div className="account-backdrop"><section className="account-dialog account-recovery" role="dialog" aria-modal="true" aria-labelledby="recovery-title">
    ...
    {finished ? <><p role="status">{finished}</p><a href="/">지도로 돌아가기</a></> : <>
      <p>같은 소셜 계정이 확인됐어요. 복구를 선택하기 전에는 로그인되지 않습니다.</p>
      {status && <><p>{status.displayName || '이전 계정'}의 계정과 제보 연결을 복구할 수 있어요. 관리자 권한은 복구되지 않으며, 필수 약관을 다시 확인합니다.</p>
        <p>삭제 예정: {new Date(status.purgeAfter.endsWith('Z') || /[+-]\d\d:\d\d$/.test(status.purgeAfter) ? status.purgeAfter : `${status.purgeAfter}+09:00`).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })} (한국시간)</p>
        <div className="recovery-actions"><button type="button" className="recovery-primary" disabled={busy} onClick={() => void decide('RESTORE')}>이전 계정 복구</button>
          {!eraseConfirmed ? <button type="button" className="recovery-delete" disabled={busy} onClick={() => setEraseConfirmed(true)}>복구 없이 즉시 삭제</button> : <><p>회원정보와 제보의 회원 연결·자유 입력 사유가 삭제되며 되돌릴 수 없어요.</p><button type="button" className="recovery-delete" disabled={busy} onClick={() => void decide('ERASE')}>확인했어요, 완전히 삭제</button></>}
        </div></>}
