  ...
  return <div className="region-site-shell">
    ...
    <main className="region-main">
      ...
      {district && province ? <section className="region-district-layout" aria-label={title}>
        <div className="region-district-map-card"><div className="region-card-heading"><span className="region-eyebrow">{r.locationMap}</span><h2>{title}</h2><p>{r.nearby} · {visible.length.toLocaleString(locale)} {r.toilets}</p></div><DistrictNaverMap district={district} toilets={toilets} locale={locale} /></div>
      ...
      </section> : <section className="region-discovery-layout"><div className="region-atlas-card"><div className="region-card-heading"><span className="region-eyebrow">{r.atlas}</span><h2>{province ? r.chooseDistrict : r.chooseProvince}</h2></div><RegionAtlas locale={locale} provinceCode={province?.code} /></div><div className="region-options-card"><div className="region-card-heading"><span className="region-eyebrow">{province ? r.districtList : r.provinceList}</span><h2>{province ? r.chooseDistrict : r.chooseProvince}</h2><p>{options.length} {province ? r.districts : r.regions}</p></div><div className="region-options-list">{options.map(option => <Link key={option.code} href={localized(regionPath(province?.code ?? option.code, province ? option.code : undefined))}><span><strong>{regionName(option, locale)}</strong><small>{option.code}</small></span><span>{option.count.toLocaleString(locale)} <small>{r.toilets}</small></span><b aria-hidden="true">↗</b></Link>)}</div></div></section>}
      <p className="region-source">{r.source}: <a href="https://github.com/DevMinGeonPark/mapcn-kr">SGIS · 행정안전부 / vuski/admdongkor / mapcn-kr</a> (CC BY 4.0). {new Date(regionSnapshot.generatedAt).toLocaleDateString(locale)} · {regionSnapshot.unassigned.toLocaleString(locale)} {r.outsideBoundary}.</p>
    </main>
