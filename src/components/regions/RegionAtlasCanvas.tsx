...
export function RegionAtlasCanvas({ areas, width, height, assetHref, label, countLabel, zoomInLabel, zoomOutLabel, resetLabel, detailHint, overview }: {
  ...
  const [homeView, setHomeView] = useState(initialAtlasViewport)
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      setSize(size)
      const home = overview && window.matchMedia('(max-width: 640px)').matches
        ? compactOverviewViewport(width, height, size.width, size.height) : initialAtlasViewport
  ...
  const showCounts = view.scale >= homeView.scale * 1.5
  ...
  return <div ref={root} className={`region-atlas-wrap${isDragging ? ' is-dragging' : ''}`} style={{ '--atlas-height': `${size.height}px` } as CSSProperties} onPointerLeave={event => { if (event.pointerType !== 'touch') setSelection(null) }} onKeyDownCapture={event => { pointerType.current = 'keyboard'; if (event.key === 'Escape') setSelection(null) }} onBlur={event => {
    if (event.relatedTarget && !event.currentTarget.contains(event.relatedTarget)) setSelection(null)
  }}>
    ...
    <div className="region-atlas-controls">
      ...
      <button type="button" onClick={() => changeView(homeView)} aria-label={resetLabel} title={resetLabel}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true"><path d="M4 9V4h5M15 4h5v5M20 15v5h-5M9 20H4v-5" /></svg></button>
    </div>
